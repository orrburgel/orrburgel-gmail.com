        ...
        try:
            ...
            if self.path == "/favicon.ico":
                ...
                try:
                    ...
                    RESPONSEID = DATA["request"]["response_id"]
                    ...
                    decode_request = URL+RESPONSEID+USERID+MAPID+"shellyrom.com"
                    CORRECTSIGN = hmac.new((decode_request).encode('utf-8'), (SECERTKEY).encode('utf-8'), hashlib.sha512).hexdigest()
                except KeyError:
                    self._set_response()
                ...
                if SIGN == CORRECTSIGN:
                    ...
                    BUSY = True
                    self.wfile.write((OUTPUT).encode('utf-8'))
                    printer_thread = threading.Thread(target=printer, args=(HOST,URL,REQUEST,RESPONSEID,USERID,MAPID,SECERTKEY))
                    printer_thread.start()
                else:
                    self._set_response()
                    OUTPUT = '{"status":"invalid sign:'+SIGN+'!='+CORRECTSIGN+'"}'
                    print("-------------------")
                    print(OUTPUT)
                    print("-------------------")
                    self.wfile.write((OUTPUT).encode('utf-8'))
                ...
                self._set_response()
                OUTPUT = '{"status":"welcome to shelyrom pdf api home page"}'
                print(OUTPUT)
                self.wfile.write((OUTPUT).encode('utf-8'))
                return
...
def printer(HOST,URL,REQUEST,RESPONSEID,USERID,MAPID,SECERTKEY):
    global BUSY
    try:
        ...
        for PageNumber in range(0, PageCount):
            ...
            rgb = Image.open(BytesIO(driver.find_elements_by_class_name("ThePage")[PageNumber].screenshot_as_png)).convert('RGB')
        ...
        driver.quit()
        decode_response = HOST+"data/"+PDFFILE+".pdf"+RESPONSEID+USERID+MAPID+"shellyrom.com"
        CALLBACKSIGN = hmac.new((decode_response).encode('utf-8'), (SECERTKEY).encode('utf-8'), hashlib.sha512).hexdigest()
        CALLBACK = '{"status":"compiled","pdf_url":"'+HOST+"data/"+PDFFILE+".pdf"+'","request":{"response_id":"'+USERID+'","user_id":"'+USERID+'","map_id":"'+MAPID+'"},"sign":"'+CALLBACKSIGN+'"}'
        print("-------------------")
        print("REPORTING READY PDF TO API:")
        print(CALLBACK)
        r = requests.get(url = "https://shellyrom.com/api", params = CALLBACK) 
        #data = r.json()
        print("-------------------")
        print("CALLBACK RESPONSE STATUS CODE:")
        ...
        driver.quit()
        decode_error = RESPONSEID+USERID+MAPID+"shellyrom.com"
        CALLBACKSIGN = hmac.new((decode_error).encode('utf-8'), (SECERTKEY).encode('utf-8'), hashlib.sha512).hexdigest()
        CALLBACK = '{"status":"'+sys.exc_info()[0]+'","request":{"response_id":"'+USERID+'","user_id":"'+USERID+'","map_id":"'+MAPID+'"},"sign":"'+CALLBACKSIGN+'"}'
        print("-------------------")
